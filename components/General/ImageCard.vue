<template>
    <div
      class="w-full my-4 lg:my-0 overflow-hidden group relative cursor-pointer"
      :style="containerHeight(photo) && containerWidth(photo)"
      >
          <img 
            class="object-cover w-full"
            :style="containerHeight(photo) && containerWidth(photo)"
            :src="photo.src.original"
            :alt="photo.photographer"
          />
            <div class="photo-info text-white hidden group-hover:block absolute origin-bottom w-full bottom-0 ">
              <div 
                class="flex justify-between text-white text-opacity-100 items-center p-2 shadow-lg">
                <div class="relative">
                  <a
                   :href="photo.photographer_url"
                   target="_blank"
                   class="inline-flex items-center photographer-menu">
                    <img
                      class="mr-2 inline object-cover w-8 h-8 rounded-full"
                      :src="photo.src.tiny"
                      :alt="photo.photographer"
                    />
                    <p class="">
                      {{ photo.photographer }} 
                    </p>
                  </a>
                  <div
                    class="hidden photographer-menu-content image-menu absolute bottom-6 origin-top-right left-0 mt-2 -mr-1 shadow-lg bg-white text-black mb-5 z-50 font-normal leading-normal text-sm w-80 no-underline break-words p-4"
                  >
                    <div class="flex justify-between">
                      <div class="inline-flex">
                        <img
                          class="mr-2 inline object-cover w-12 h-12 rounded-full"
                          :src="photo.src.original" :alt="photo.photographer"
                        />
                        <div>
                          <p>
                            {{ photo.photographer }} 
                          </p>
                          <p>4.2k followers</p>
                        </div>
                      </div>
                      <button class="bg-green-500 text-white px-4 py-2 h-12">Follow</button>
                    </div>

                    <div class="lg:grid lg:grid-cols-3 lg:gap-3 my-2">
                      <div class="w-full lg:my-0 overflow-hidden">
                        <nuxt-link class="" to="/">
                          <img 
                          class="object-cover w-full h-20" 
                          :src="photo.src.original" :alt="photo.photographer"
                          />
                        </nuxt-link>
                      </div>
                      <div class="w-full lg:my-0 overflow-hidden">
                        <nuxt-link class="" to="/">
                          <img 
                          class="object-cover w-full h-20" 
                          :src="photo.src.original" :alt="photo.photographer"
                          />
                        </nuxt-link>
                      </div>
                      <div class="w-full lg:my-0 overflow-hidden">
                        <nuxt-link class="" to="/">
                          <img 
                          class="object-cover w-full h-20" 
                          :src="photo.src.original" :alt="photo.photographer"
                          />
                        </nuxt-link>
                      </div>
                    </div>

                    <div>
                      <div class="inline-flex flex">
                        <div class="m-1 inline-flex">
                          <svg 
                            viewBox="0 0 20 20"
                            fill="currentColor"
                            stroke="currentColor"
                            class="h-4 w-4 mr-1 mt-1"
                          >
                            <path 
                              d="M10,6.978c-1.666,0-3.022,1.356-3.022,3.022S8.334,13.022,10,13.022s3.022-1.356,3.022-3.022S11.666,6.978,10,6.978M10,12.267c-1.25,0-2.267-1.017-2.267-2.267c0-1.25,1.016-2.267,2.267-2.267c1.251,0,2.267,1.016,2.267,2.267C12.267,11.25,11.251,12.267,10,12.267 M18.391,9.733l-1.624-1.639C14.966,6.279,12.563,5.278,10,5.278S5.034,6.279,3.234,8.094L1.609,9.733c-0.146,0.147-0.146,0.386,0,0.533l1.625,1.639c1.8,1.815,4.203,2.816,6.766,2.816s4.966-1.001,6.767-2.816l1.624-1.639C18.536,10.119,18.536,9.881,18.391,9.733 M16.229,11.373c-1.656,1.672-3.868,2.594-6.229,2.594s-4.573-0.922-6.23-2.594L2.41,10l1.36-1.374C5.427,6.955,7.639,6.033,10,6.033s4.573,0.922,6.229,2.593L17.59,10L16.229,11.373z"
                              />
                            <path 
                              d="M405.333,469.333H106.667c-11.782,0-21.333,9.551-21.333,21.333S94.885,512,106.667,512h298.667 c11.782,0,21.333-9.551,21.333-21.333S417.115,469.333,405.333,469.333z"
                            />
                          </svg>
                          <span>200k</span>
                        </div>
                        <div class="m-1 inline-flex">
                          <svg 
                            viewBox="0 0 512 512"
                            fill="currentColor"
                            stroke="currentColor"
                            class="h-4 w-4 mr-1 mt-1"
                          >
                            <path 
                              d="M240.915,420.418c0.004,0.004,0.008,0.007,0.012,0.011c0.493,0.492,1.012,0.959,1.551,1.402
                                c0.247,0.203,0.508,0.379,0.762,0.57c0.303,0.227,0.6,0.462,0.915,0.673c0.304,0.203,0.619,0.379,0.931,0.565
                                c0.286,0.171,0.565,0.35,0.859,0.507c0.318,0.17,0.645,0.314,0.97,0.467c0.306,0.145,0.608,0.298,0.922,0.428
                                c0.315,0.13,0.637,0.236,0.957,0.35c0.337,0.121,0.669,0.25,1.013,0.354c0.32,0.097,0.645,0.168,0.969,0.249
                                c0.351,0.089,0.698,0.187,1.056,0.258c0.375,0.074,0.753,0.118,1.13,0.172c0.311,0.044,0.618,0.104,0.933,0.135
                                c1.4,0.138,2.811,0.138,4.211,0c0.315-0.031,0.622-0.09,0.933-0.135c0.377-0.054,0.755-0.098,1.13-0.172
                                c0.358-0.071,0.705-0.169,1.056-0.258c0.323-0.081,0.649-0.152,0.969-0.249c0.344-0.104,0.677-0.233,1.013-0.354
                                c0.32-0.115,0.642-0.22,0.957-0.35c0.314-0.13,0.616-0.283,0.922-0.428c0.325-0.153,0.652-0.297,0.97-0.467
                                c0.294-0.157,0.573-0.337,0.859-0.507c0.312-0.186,0.627-0.362,0.931-0.565c0.315-0.211,0.612-0.446,0.915-0.673
                                c0.254-0.19,0.515-0.367,0.762-0.57c0.539-0.443,1.058-0.909,1.551-1.402c0.004-0.004,0.008-0.007,0.012-0.011l149.333-149.333
                                c8.331-8.331,8.331-21.839,0-30.17s-21.839-8.331-30.17,0L277.333,353.83V21.333C277.333,9.551,267.782,0,256,0
                                s-21.333,9.551-21.333,21.333V353.83L121.752,240.915c-8.331-8.331-21.839-8.331-30.17,0s-8.331,21.839,0,30.17L240.915,420.418z"
                              />
                            <path 
                              d="M405.333,469.333H106.667c-11.782,0-21.333,9.551-21.333,21.333S94.885,512,106.667,512h298.667 c11.782,0,21.333-9.551,21.333-21.333S417.115,469.333,405.333,469.333z"
                            />
                          </svg>
                          <span>200k</span>
                        </div>
                        <div class="m-1 inline-flex">
                          <svg 
                            viewBox="0 0 20 20"
                            fill="currentColor"
                            stroke="currentColor"
                            class="h-4 w-4 mr-1 mt-1"
                          >
                            <path 
                              d="M13.22,2.984c-1.125,0-2.504,0.377-3.53,1.182C8.756,3.441,7.502,2.984,6.28,2.984c-2.6,0-4.714,2.116-4.714,4.716c0,0.32,0.032,0.644,0.098,0.96c0.799,4.202,6.781,7.792,7.46,8.188c0.193,0.111,0.41,0.168,0.627,0.168c0.187,0,0.376-0.041,0.55-0.127c0.011-0.006,1.349-0.689,2.91-1.865c0.021-0.016,0.043-0.031,0.061-0.043c0.021-0.016,0.045-0.033,0.064-0.053c3.012-2.309,4.6-4.805,4.6-7.229C17.935,5.1,15.819,2.984,13.22,2.984z M12.544,13.966c-0.004,0.004-0.018,0.014-0.021,0.018s-0.018,0.012-0.023,0.016c-1.423,1.076-2.674,1.734-2.749,1.771c0,0-6.146-3.576-6.866-7.363C2.837,8.178,2.811,7.942,2.811,7.7c0-1.917,1.554-3.47,3.469-3.47c1.302,0,2.836,0.736,3.431,1.794c0.577-1.121,2.161-1.794,3.509-1.794c1.914,0,3.469,1.553,3.469,3.47C16.688,10.249,14.474,12.495,12.544,13.966z"
                              />
                            <path 
                              d="M405.333,469.333H106.667c-11.782,0-21.333,9.551-21.333,21.333S94.885,512,106.667,512h298.667 c11.782,0,21.333-9.551,21.333-21.333S417.115,469.333,405.333,469.333z"
                            />
                          </svg>
                          <span>200k</span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>

                <div class="inline-flex flex">
                  <a href="#" class="no-underline mx-2">
                    <svg  viewBox="0 0 512 512" fill="currentColor" class="h-6 w-6">
                      <path 
                        d="M240.915,420.418c0.004,0.004,0.008,0.007,0.012,0.011c0.493,0.492,1.012,0.959,1.551,1.402
                          c0.247,0.203,0.508,0.379,0.762,0.57c0.303,0.227,0.6,0.462,0.915,0.673c0.304,0.203,0.619,0.379,0.931,0.565
                          c0.286,0.171,0.565,0.35,0.859,0.507c0.318,0.17,0.645,0.314,0.97,0.467c0.306,0.145,0.608,0.298,0.922,0.428
                          c0.315,0.13,0.637,0.236,0.957,0.35c0.337,0.121,0.669,0.25,1.013,0.354c0.32,0.097,0.645,0.168,0.969,0.249
                          c0.351,0.089,0.698,0.187,1.056,0.258c0.375,0.074,0.753,0.118,1.13,0.172c0.311,0.044,0.618,0.104,0.933,0.135
                          c1.4,0.138,2.811,0.138,4.211,0c0.315-0.031,0.622-0.09,0.933-0.135c0.377-0.054,0.755-0.098,1.13-0.172
                          c0.358-0.071,0.705-0.169,1.056-0.258c0.323-0.081,0.649-0.152,0.969-0.249c0.344-0.104,0.677-0.233,1.013-0.354
                          c0.32-0.115,0.642-0.22,0.957-0.35c0.314-0.13,0.616-0.283,0.922-0.428c0.325-0.153,0.652-0.297,0.97-0.467
                          c0.294-0.157,0.573-0.337,0.859-0.507c0.312-0.186,0.627-0.362,0.931-0.565c0.315-0.211,0.612-0.446,0.915-0.673
                          c0.254-0.19,0.515-0.367,0.762-0.57c0.539-0.443,1.058-0.909,1.551-1.402c0.004-0.004,0.008-0.007,0.012-0.011l149.333-149.333
                          c8.331-8.331,8.331-21.839,0-30.17s-21.839-8.331-30.17,0L277.333,353.83V21.333C277.333,9.551,267.782,0,256,0
                          s-21.333,9.551-21.333,21.333V353.83L121.752,240.915c-8.331-8.331-21.839-8.331-30.17,0s-8.331,21.839,0,30.17L240.915,420.418z"
                        />
                      <path 
                        d="M405.333,469.333H106.667c-11.782,0-21.333,9.551-21.333,21.333S94.885,512,106.667,512h298.667 c11.782,0,21.333-9.551,21.333-21.333S417.115,469.333,405.333,469.333z"
                      />
                    </svg>
                  </a>
                    <a href="#" class="no-underline mx-2">
                    <svg viewBox="0 0 20 20" fill="currentColor" class="h-6 w-6">
                      <path
                        d="M14.613,10c0,0.23-0.188,0.419-0.419,0.419H10.42v3.774c0,0.23-0.189,0.42-0.42,0.42s-0.419-0.189-0.419-0.42v-3.774H5.806c-0.23,0-0.419-0.189-0.419-0.419s0.189-0.419,0.419-0.419h3.775V5.806c0-0.23,0.189-0.419,0.419-0.419s0.42,0.189,0.42,0.419v3.775h3.774C14.425,9.581,14.613,9.77,14.613,10 M17.969,10c0,4.401-3.567,7.969-7.969,7.969c-4.402,0-7.969-3.567-7.969-7.969c0-4.402,3.567-7.969,7.969-7.969C14.401,2.031,17.969,5.598,17.969,10 M17.13,10c0-3.932-3.198-7.13-7.13-7.13S2.87,6.068,2.87,10c0,3.933,3.198,7.13,7.13,7.13S17.13,13.933,17.13,10"
                        />
                    </svg>
                  </a>
                    <a href="#" class="no-underline mx-2">
                    <svg viewBox="0 0 20 20" fill="currentColor" class="h-6 w-6">
                      <path
                        d="M13.22,2.984c-1.125,0-2.504,0.377-3.53,1.182C8.756,3.441,7.502,2.984,6.28,2.984c-2.6,0-4.714,2.116-4.714,4.716c0,0.32,0.032,0.644,0.098,0.96c0.799,4.202,6.781,7.792,7.46,8.188c0.193,0.111,0.41,0.168,0.627,0.168c0.187,0,0.376-0.041,0.55-0.127c0.011-0.006,1.349-0.689,2.91-1.865c0.021-0.016,0.043-0.031,0.061-0.043c0.021-0.016,0.045-0.033,0.064-0.053c3.012-2.309,4.6-4.805,4.6-7.229C17.935,5.1,15.819,2.984,13.22,2.984z M12.544,13.966c-0.004,0.004-0.018,0.014-0.021,0.018s-0.018,0.012-0.023,0.016c-1.423,1.076-2.674,1.734-2.749,1.771c0,0-6.146-3.576-6.866-7.363C2.837,8.178,2.811,7.942,2.811,7.7c0-1.917,1.554-3.47,3.469-3.47c1.302,0,2.836,0.736,3.431,1.794c0.577-1.121,2.161-1.794,3.509-1.794c1.914,0,3.469,1.553,3.469,3.47C16.688,10.249,14.474,12.495,12.544,13.966z"
                        />
                    </svg>
                  </a>
                </div>
              </div>
            </div>
        </div>

</template>

<script>
export default {
    name: "image-card",

    props: {
      photo: {
        type: Object,
        default: {},
        required: true
      }
    },

    methods: {
      containerHeight(photo) {
        let height = '600px';
        if (photo.height) height = photo.height;
        return 'background-color: ' + height;
      },
      containerWidth(photo) {
        let width = '450px';
        if (photo.width) width = photo.width;
        return 'background-color: ' + width;
      }
      
    }
}
</script>

<style>

</style>
